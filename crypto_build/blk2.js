var Derive = (() => {
  ...
  __export(derive_exports, {
    ...
    derivePathAddresses: () => derivePathAddresses
  });
  ...
  function twistedEdwards(curveDef) {
    ...
    class Point3 {
      ...
      static fromAffine(p) {
        ...
        if (!in0MaskRange(x) || !in0MaskRange(y))
          throw new Error("invalid affine point");
        return new Point3(x, y, _1n6, modP2(x * y));
      }
      static normalizeZ(points) {
    ...
    Point3.BASE = new Point3(CURVE.Gx, CURVE.Gy, _1n6, modP2(CURVE.Gx * CURVE.Gy));
    Point3.ZERO = new Point3(_0n6, _1n6, _1n6, _0n6);
  ...
  function bs58encode(bytes3) {
    let zeros = 0;
    while (zeros < bytes3.length && bytes3[zeros] === 0) zeros++;
    const digits = [];
    let len = 0;
    for (let i = zeros; i < bytes3.length; i++) {
      let carry = bytes3[i];
      for (let j = 0; j < len; j++) {
        carry += digits[j] << 8;
        digits[j] = carry % 58;
  ...
  function hash1602(buf) {
    return ripemd160(sha256(buf));
  }
  function taggedHash2(tag, data) {
    const th = sha256(utf8(tag));
    return sha256(concatBytes3(th, th, data));
  }
  function base58check2(payload) {
    const cs = sha256(sha256(payload)).slice(0, 4);
    return bs58encode(concatBytes3(payload, cs));
  }
  function btcLegacy(pub) {
    return base58check2(concatBytes3(new Uint8Array([0]), hash1602(pub)));
  }
  function btcNested(pub) {
    const h = hash1602(pub);
    const redeem = concatBytes3(new Uint8Array([0, 20]), h);
    return base58check2(concatBytes3(new Uint8Array([5]), hash1602(redeem)));
  }
  function btcSegwit(pub) {
    const h = hash1602(pub);
    return import_bech32.bech32.encode("bc", [0, ...import_bech32.bech32.toWords(h)]);
  }
  function btcTaproot(priv) {
    const internalX = schnorr.getPublicKey(priv);
    const tweak = taggedHash2("TapTweak", concatBytes3(internalX, new Uint8Array(32)));
    const fullPub = secp256k1.getPublicKey(priv, false);
    const P = secp256k1.ProjectivePoint.fromHex(fullPub);
    const tG = secp256k1.ProjectivePoint.fromPrivateKey(tweak);
    const Q = P.add(tG);
    const xQ = Q.toRawBytes(true).slice(1);
    return import_bech32.bech32m.encode("bc", [1, ...import_bech32.bech32m.toWords(xQ)]);
  }
  function btcWIF(priv) {
    return base58check2(concatBytes3(new Uint8Array([128]), priv, new Uint8Array([1])));
  }
  ...
  function ethChecksum(addr20) {
    const lower = bytesToHex3(addr20);
    const hash3 = keccak_256(utf8(lower));
    let out = "0x";
    for (let i = 0; i < 40; i++) {
      const c = lower[i];
      const nib = hash3[i >> 1] >> (i % 2 ? 0 : 4) & 15;
      out += nib >= 8 && c >= "a" && c <= "f" ? c.toUpperCase() : c;
    }
    return out;
  }
  function deriveEd25519Full(seed, path) {
    let I = hmac2(sha5122, utf8("ed25519 seed"), seed);
    let k = I.slice(0, 32), c = I.slice(32);
    for (const seg of path.replace(/^m\//, "").split("/")) {
      const hardened = seg.endsWith("'");
      const idx = parseInt(seg.replace("'", ""), 10) + (hardened ? 2147483648 : 0);
      I = hmac2(sha5122, c, concatBytes3(new Uint8Array([0]), k, u322(idx)));
      k = I.slice(0, 32);
      c = I.slice(32);
    }
    return k;
  }
  function solAddress(priv) {
    return bs58encode(ed25519.getPublicKey(priv));
  }
  function suiAddress(priv) {
    return "0x" + bytesToHex3(ed25519.getPublicKey(priv));
  }
  var COINS = {
    BTC: {
      type: "secp256k1",
      paths: [
        { label: "Legacy \xB7 P2PKH", path: "m/44'/0'/0'/0/0", kind: "legacy" },
        { label: "Nested SegWit \xB7 P2SH-P2WPKH", path: "m/49'/0'/0'/0/0", kind: "nested" },
        { label: "Native SegWit \xB7 P2WPKH", path: "m/84'/0'/0'/0/0", kind: "segwit" },
        { label: "Taproot \xB7 P2TR", path: "m/86'/0'/0'/0/0", kind: "taproot" }
      ]
    },
    ETH: {
      type: "secp256k1",
      paths: [{ label: "Ethereum \xB7 m/44'/60'/0'/0/0", path: "m/44'/60'/0'/0/0", kind: "eth" }]
    },
    SOL: {
      type: "ed25519",
      paths: [{ label: "Solana \xB7 m/44'/501'/0'/0/0", path: "m/44'/501'/0'/0/0", kind: "sol" }]
    },
    SUI: {
      type: "ed25519",
      paths: [{ label: "Sui \xB7 m/44'/784'/0'/0/0", path: "m/44'/784'/0'/0/0", kind: "sui" }]
    }
  };
  function addressFor(kind, priv, pub) {
    switch (kind) {
      case "legacy":
        return btcLegacy(pub);
      case "nested":
        return btcNested(pub);
      case "segwit":
        return btcSegwit(pub);
      case "taproot":
        return btcTaproot(priv);
      case "eth":
        return ethAddress(priv);
      case "sol":
        return solAddress(priv);
      case "sui":
        return suiAddress(priv);
    }
    return "";
  }
  function deriveAccounts(mnemonic, coin) {
  ...
  function derivePathAddresses(mnemonic, coin, path, start, count) {
    const seed = mnemonicToSeedSync(mnemonic, "");
    const cfg = COINS[coin];
    ...
    if (cfg.type === "secp256k1") {
      ...
      for (let i = start; i < start + count; i++) {
        ...
        const priv = hd.privateKey, pub = hd.publicKey;
        results.push({ index: i, address: addressFor(kind, priv, pub), privHex: "0x" + bytesToHex3(priv) });
      }
    } else {
      for (let i = start; i < start + count; i++) {
        const last = lastHardened ? i + "'" : String(i);
        ...
        results.push({ index: i, address: addressFor(kind, priv, null), privHex: "0x" + bytesToHex3(priv) });
      }
    }
    return results;
  }
  var _util = { bytesToHex: bytesToHex3, sha256, addressFor };
  return __toCommonJS(derive_exports);
})();
